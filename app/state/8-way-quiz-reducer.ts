import { type Formation, EIGHT_WAY_BLOCKS, EIGHT_WAY_RANDOMS, isRandom } from "~/data/formations";
...
function generateQuestions(questionSet: QuestionSet[]): Question[] {
  const formationsToInclude = [
    ...(questionSet.includes(QuestionSet.RANDOMS) ? EIGHT_WAY_RANDOMS : []),
    ...(questionSet.includes(QuestionSet.BLOCKS_A) ? EIGHT_WAY_BLOCKS : []),
  ]
...
function generateMultipleChoiceAnswers(actualAnswer: Formation): Formation[] {
  const answerPool: Formation[] = (isRandom(actualAnswer) ? EIGHT_WAY_RANDOMS : EIGHT_WAY_BLOCKS)
