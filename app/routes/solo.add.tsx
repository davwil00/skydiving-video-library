...
export const action = async ({ request }: Route.ActionArgs) => {
    ...
    const fileNames = pendingDir.map(
        (file) => `/video-data/solo/library/${date}/${file.name}`,
    );
...
export const loader = async () => {
    const pendingDir = await getAllFiles();
    const files = pendingDir.map(
        (file) => `/video-data/solo/pending/${file.name}`,
    );
    return { files };
};
    ...
    if (files.length === 0) {
        return (
            <div>
                ...
                <p>
                    No pending solo session videos found. Please upload videos
                    to the pending folder.
                </p>
            </div>
        );
    }
