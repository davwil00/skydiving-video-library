import type { FlightFormation } from 'prisma/generated/client';
...
export default function CompareFlights() {
    ...
    const togglePlay = () => {
        Promise.all([
            video1Ref.current?.paused
                ? video1Ref.current.play()
                : video1Ref.current?.pause(),
            video2Ref.current?.paused
                ? video2Ref.current.play()
                : video2Ref.current?.pause(),
        ]).then(() => setIsPlaying((prev) => !prev));
    };

    const formationsToString = (formations: Array<FlightFormation>) =>
        formations.map((formation) => formation.formationId).join(' ');

    return (
        <div className="flex align-center">
            <div>
                <h2>
                    Flight 1: {formatDate(flight1.session.date)}{' '}
                    {formationsToString(flight1.formations)}
                </h2>
                <video
                    ref={video1Ref}
                    controls
                ...
                <h2>
                    Flight 2 {formatDate(flight1.session.date)}{' '}
                    {formationsToString(flight1.formations)}
                </h2>
                <video
                    ref={video2Ref}
            ...
            <div className="content-center">
                <button className="btn m-4" onClick={togglePlay} type="button">
