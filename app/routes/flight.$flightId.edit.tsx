...
import {
    ...
    editFlightReducer,
} from '~/state/edit-flight-reducer';
...
export default function EditFlight() {
    ...
    const scoresPerRound = calculateScoresPerRound(
        flight.formations.map((formation) => formation.formationId),
    );
            ...
            roundElts.push(
                <tr key={`round-${round}`}>
                    ...
                    {flight.formations.map((formation, formationIdx) => {
                        const id =
                            round * flight.formations.length + formationIdx;
                        if (isRandomFormation(formation.formationId)) {
                            scoreIdx += 1;
                            return scoreIcon(
                                `formation-${formationIdx}-round-${round}`,
                                `${id}`,
                        ...
                        return (
                            <>
                                {scoreIcon(
                                    `formation-${formationIdx}-round-${round}-A`,
                                    `${id}-A`,
                                    formation.formationId,
                                    scoreIdx - 2,
                                )}
                                {scoreIcon(
                                    `formation-${formationIdx}-round-${round}-B`,
                                    `${id}-B`,
                                    formation.formationId,
                                    scoreIdx - 1,
                                )}
        ...
        <div className="flex justify-between items-start form-light gap-4">
            <div>
                <form method="post" className="flex flex-col">
                    ...
                    <input
                        type="hidden"
                        value={flight.sideVideoUrl || ''}
                        name="sideVideoUrl"
                    />
                    <input
                        type="hidden"
                        value={flight.topVideoUrl || ''}
                        name="topVideoUrl"
                    />
                    <label className="">
                    ...
                    <button className="btn btm-primary mt-3" type="submit">
                        Submit
                    </button>
                </form>
                ...
                <form
                    method="post"
                    action={`/flight/${flight.id}/edit-scores`}
                ...
                >
                    ...
                    <table className="table table-auto border-1 border-black">
                            ...
                            <tr className="border-black">
                                ...
                                {flight.formations.map((formation) =>
                                    isRandomFormation(formation.formationId) ? (
                                        <th
                                            key={`formation-${formation.formationId}`}
                        ...
                        <tbody>{makeRounds()}</tbody>
                    </table>
                    <button
                        type="button"
                        className="btn btn-outline mt-2"
                        onClick={() =>
                            dispatch({ type: 'addRound', scoresPerRound })
                        }
                    >
                        Add Round
                    </button>
