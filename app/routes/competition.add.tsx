...
export default function AddCompetition() {
    ...
    const [linkedSessions, setLinkedSessions] = useState<Set<SessionIdAndDate>>(
        new Set(),
    );
    ...
    const addSession = () => {
        const selectedSession = sessions.find(
            (session) => session.id === sessionPicker.current?.value,
        );
        if (selectedSession) {
        ...
        <div>
            ...
            <form method="post" className="form-light flex flex-col">
                ...
                <div className="my-2">
                    ...
                    <ul className="list">
                        {Array.from(linkedSessions).map((linkedSession) => (
                            <li
                                key={`linkedSession${linkedSession.id}`}
                                className="list-row items-center"
                            >
                                <input
                                    type="hidden"
                                    name={`linkedSession`}
                                    value={linkedSession.id}
                                />
                                <div>{formatDate(linkedSession.date)}</div>
                                <button
                                    type="button"
                                    className="btn btn-square btn-xs btn-outline"
                                    onClick={() => removeSession(linkedSession)}
                                >
                                    x
                                </button>
                            </li>
                ...
                <button
                    type="submit"
                    className="btn mt-4 max-w-xs"
                    disabled={linkedSessions.size === 0}
                >
                    Add
                </button>
            </form>
